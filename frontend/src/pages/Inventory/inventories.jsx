...
const Home = () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/notifications`, {
        ...
        headers: { Authorization: `Bearer ${token}` }
          ...
          handleApiError(error, 'Failed to send low inventory notification');
        }
  };
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/inventory`, {
      ...
      .then((response) => {
        const inventories = response.data.inventories || response.data.data || [];
        // sort newest first by createdAt
        inventories.sort((a, b) => new Date(b.created_at || b.createdAt) - new Date(a.created_at || a.createdAt));
        ...
        .catch((error) => {
          ...
          setLoading(false);
          handleApiError(error, 'Failed to fetch inventory data');
        });
  }, []);
  ...
  const handleSearch = () => {
    if (searchInput.trim() === '') {
      ...
      const filtered = originalInventory.filter(item =>
        (item.inventoryid || item.inventory_id) && (item.inventoryid || item.inventory_id).toLowerCase().includes(searchInput.toLowerCase())
      );
...
const handleReportGeneration = () => {
  ...
  useEffect(() => {
    ...
    const total = originalInventory.reduce((sum, item) => sum + (item.quantity || item.weight_kg || 0), 0);
  ...
  const chartData = {
    labels: inventory.map(item => item.inventoryid || item.inventory_id || 'N/A'),
    datasets: [
      {
        label: 'Quantity',
        data: inventory.map(item => item.quantity || item.weight_kg || 0),
                ...
                <tbody>
                  {inventory.slice(0, visibleCount).map((item, index) => (
                    <tr key={item.id} className="border-t">
                      ...
                      <td className="px-6 py-3 font-semibold text-black">{item.inventoryid || item.inventory_id || '-'}</td>
                      <td className="px-6 py-3">{item.quantity || item.weight_kg || '-'}</td>
