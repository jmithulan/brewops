...
const TeaFactoryPayment = () => {
  ...
  const fetchSuppliers = async () => {
    ...
    const res = await axios.get('http://localhost:4323/api/suppliers/active', {
  ...
  const fetchPayments = async () => {
    const token = localStorage.getItem('jwtToken');
    const res = await axios.get('http://localhost:4323/api/payments', {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.data && res.data.success) {
  ...
  const fetchPaymentStatistics = async () => {
    try {
      const token = localStorage.getItem('jwtToken');
      const res = await axios.get('http://localhost:4323/api/payments/statistics', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data && res.data.success) {
  ...
  const fetchDeliveryRecords = async () => {
    try {
      const token = localStorage.getItem('jwtToken');
      const res = await axios.get('http://localhost:4323/api/deliveries', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data && res.data.success) {
    ...
    if (supplierForm.name && supplierForm.phone && supplierForm.bankAccount && supplierForm.rate) {
      (async () => {
        try {
          ...
          const res = await axios.post('http://localhost:4323/api/suppliers', payload, { headers: { Authorization: `Bearer ${token}` } });
          if (res.data && res.data.success) {
  ...
  const processPayment = (supplierId, amount) => {
    (async () => {
      try {
        ...
        let res;
        try {
          res = await axios.post('http://localhost:4323/api/payments/monthly', payload, { 
            headers: { Authorization: `Bearer ${token}` } 
          });
        } catch (err) {
          ...
          const updatePromises = monthlyDeliveries
            ...
            .map(delivery => 
              axios.put(`http://localhost:4323/api/deliveries/${delivery.id}`, {
                ...delivery,
                payment_status: 'paid',
    ...
    if (selectedSupplierForSpotCash && spotCashAmount && parseFloat(spotCashAmount) > 0 && paymentMethod) {
      (async () => {
        try {
          ...
          let res;
          try {
            res = await axios.post('http://localhost:4323/api/payments/spot-cash', payload, { 
              headers: { Authorization: `Bearer ${token}` } 
            });
          } catch (err) {
            // Fallback to updating delivery records directly
            ...
            if (matchingDeliveries.length > 0) {
              ...
              res = await axios.put(`http://localhost:4323/api/deliveries/${delivery.id}`, {
                ...delivery,
                payment_status: 'paid',
                payment_date: new Date().toISOString().split('T')[0]
              }, { headers: { Authorization: `Bearer ${token}` } });
