...
export async function loginUser(req, res) {
  ...
  try {
    ...
    console.log("User found:", {
      ...
      google_id: user.google_id ? 'present' : 'not set'
    });

    // Check if user has no password (OAuth user trying regular login)
    if (!user.password_hash) {
      console.log("Login failed: User has no password set (likely OAuth user)");
      return res.status(401).json({ 
        success: false,
