...
import { Layout, PageHeader, FormInput, Button, Modal } from '../../components/shared/index.js';
...
const SupplierManagement = () => {
  ...
  const {
    ...
    },
    {
      ...
      contact_number: [validators.required('Contact number is required'), validators.phone()],
      nic_number: [validators.required('NIC number is required'), validators.nicNumber()],
      address: [validators.required('Address is required'), validators.minLength(10, 'Address must be at least 10 characters')],
      bank_account_number: [validators.required('Bank account number is required'), validators.bankAccount()],
      bank_name: [validators.required('Bank name is required')],
  ...
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const suppliersData = result.data.suppliers || result.data.data || result.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        handleApiError(new Error(result.error || 'Failed to save supplier'));
      }
    } catch (error) {
      console.error('Error saving supplier:', error);
      handleApiError(error, 'An error occurred while saving the supplier');
    }
    ...
    if (window.confirm(`Are you sure you want to delete ${supplier.name}?`)) {
      try {
        ...
        if (result.success) {
          ...
          handleApiError(new Error(result.error || 'Failed to delete supplier'));
        }
      } catch (error) {
        console.error('Error deleting supplier:', error);
        handleApiError(error, 'An error occurred while deleting the supplier');
      }
    }
  };
